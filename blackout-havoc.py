...
class CmdShell( Command ):
    ...
    Help        =   "SHell execute command via cmd"
    NeedAdmin   =    False
    Params      = [
        CommandParam(
            name="argument",
...
class Blackout(AgentType):
    ...
    def response( self, response: dict ) -> bytes:
        ...
        if response[ "Agent" ] == None:
            ...
            elif Command == COMMAND_SHELL:
                out = response_parser.parse_bytes()
                self.console_message( AgentID, "Good", "Callback", out )
                self.console_message( AgentID, "Good", f"Process create succefully:", out )
